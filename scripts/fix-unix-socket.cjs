...
function isUnix(addr) {
  return typeof addr === 'string' &&
    (addr.charCodeAt(0) === 0 || addr.endsWith('.sock'));
}
function readReg() {
  try { return JSON.parse(fs.readFileSync(REGISTRY, 'utf8')); } catch { return {}; }
}
function writeReg(data) {
  try { fs.writeFileSync(REGISTRY, JSON.stringify(data)); } catch {}
}
...
function patchedConnect(addr, ...args) {
  ...
  function tryConnect() {
    ...
    if (r[key]) {
      ...
      sock.destroy(Object.assign(
        new Error(`[wsl2-fix] registry timeout: ${key}`),
        { code: 'ENOENT' }
      ));
    }
  }
